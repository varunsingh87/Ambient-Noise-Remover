...
public class MatrixCovarianceOperationTest {
    ...
    public void testCompute3D() {
        ...
        assertEquals(
                new Matrix(
                        new double[][] { { 0.02, 0.006, 0.0022 }, { 0.006, 0.012, 0.002 }, { 0.0022, 0.002, 0.0006 } }),
                generatedCovarianceMatrix);
    }

    // Merge together using multiple datasets feature
    @Test
    public void testCompute3DCaseTwo() {
        Vector set1 = Vector.row(90, 80, 40);
        Vector set2 = Vector.row(90, 60, 80);
        Vector set3 = Vector.row(60, 50, 70);
        Vector set4 = Vector.row(30, 40, 70);
        Vector set5 = Vector.row(30, 20, 90);

        Matrix generatedCovarianceMatrix = new MatrixCovarianceOperation(set1, set2, set3, set4, set5).compute();
        Matrix expected = new Matrix(
                new double[][] { { 720, 480.0, -240.0 }, { 480.0, 400.0, -280.0 }, { -240.0, -280.0, 280.0 } });
        assertEquals(expected, generatedCovarianceMatrix);
    }
}
